...
describe('odataV4Parser', function () {
    ...
    it('should propperly parse search with single AND', function () {
        ...
        expect(token.raw).to.be.equal(searchQuery);
        expect(token.value.options[0].type).to.be.equal("Search");
        expect(token.value.options[0].value.type).to.be.equal("SearchAndExpression");
        expect(token.value.options[0].value.value.left.value).to.be.equal("blue");
        expect(token.value.options[0].value.value.left.type).to.be.equal("SearchPhrase");
        expect(token.value.options[0].value.value.right.value).to.be.equal("red");
        expect(token.value.options[0].value.value.right.type).to.be.equal("SearchPhrase");
    });
    ...
    it('should propperly parse search with double AND', function () {
        ...
        //   expect(searchOptionsValue).to.be.deep.equal(expectPoC);
        expect(token.value.options[0].value.type).to.be.equal("SearchAndExpression");
        expect(token.value.options[0].value.value.left.value).to.be.equal("yellow");
        expect(token.value.options[0].value.value.left.type).to.be.equal("SearchPhrase");
        expect(token.value.options[0].value.value.right.raw).to.be.equal("blue AND green");
        expect(token.value.options[0].value.value.right.value.left.value).to.be.equal("blue");
        expect(token.value.options[0].value.value.right.value.right.value).to.be.equal("green");
        expect(token.value.options[0].value.value.right.value.left.type).to.be.equal("SearchPhrase");
        expect(token.value.options[0].value.value.right.value.right.type).to.be.equal("SearchPhrase");
    });
